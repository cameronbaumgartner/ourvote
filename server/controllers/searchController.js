...
// insert additional ' before apostrophes to escape them
...
const assembleSql = (obj) => {
  ...
  for (let i = 0; i < offices.length; i += 1) {
    ...
    for (let j = 0; j < offices[i].officialIndices.length; j++) {
      ...
      str += officials[k].photoUrl
        ? "'" + officials[k].photoUrl + "', "
        : "null, ";
      str += officials[k].urls ? "'" + officials[k].urls[0] + "', " : "null, ";
      str += "'" + officials[k].phones[0] + "', ";
      str += officials[k].emails
        ? "'" + escapeQuotes(officials[k].emails[0]) + "'"
        : "null";
      str += "), \n";
    }
  }
...
searchController.upsertByAddress = (req, res, next) => {
  ...
  fetch(
    "https://www.googleapis.com/civicinfo/v2/representatives?" + paramString
  )
    .then((res) => res.json())
    .then((data) => {
    ...
    .catch((err) =>
      console.error("Error querying Google Civic Info API.", err)
    );
};
